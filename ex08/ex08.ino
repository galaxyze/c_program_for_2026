 ...
 *   3. 布防后的前 800ms 忽略触摸读数，避免点击布防瞬间误触发。
 *   4. 忽略窗口结束后，只要触摸 GPIO4，就锁定报警。
 *   5. 报警触发后，LED 高频闪烁并锁定，即使手松开也不会停止。
 *   6. 只有点击网页“撤防 Disarm”，LED 才熄灭，报警状态才复位。
 */
...
const int TOUCH_MARGIN = 18;                 // 触摸阈值余量，作为一种阈值计算参考。
const int TOUCH_RATIO_PERCENT = 80;          // 触摸阈值比例，阈值约为未触摸基线的 80%。
const unsigned long ARM_IGNORE_MS = 800;     // 点击布防后的忽略时间，防止布防瞬间误报警。
const unsigned long ALARM_INTERVAL_MS = 80;  // 报警闪烁间隔，每 80ms 翻转一次 LED。
...
unsigned long armedAtMillis = 0;    // 记录点击布防的时间，用来实现布防后的短暂忽略窗口。
...
<head>
  ...
  <style>
    ...
    .touch { margin-bottom: 18px; color: #4b5563; }
...
<body>
  <main>
    ...
    <div id="touch" class="touch">触摸值：--</div>
  ...
  <script>
    ...
    function render(data) {
      ...
      touchBox.textContent = '触摸值：' + data.touch + ' / 阈值：' + data.threshold;
    }
    function refresh() {
...
void handleArm() {                 // 处理网页点击“布防 Arm”后的 /arm 请求。
  ...
  armedAtMillis = millis();        // 记录布防时间，后续 800ms 内不允许触发报警。
...
void handleStatus() {              // 处理网页轮询 /status 的请求。
  ...
  json += ",\"threshold\":";   // 添加 threshold 字段名。
  json += touchThreshold;          // 返回当前触摸阈值，便于判断是否达到触发条件。
  json += "}";                   // JSON 对象结束。
  server.send(200, "application/json", json); // 把状态返回给网页。
}

void updateAlarm() {  // 按“未布防 -> 布防忽略窗口 -> 已布防监视 -> 报警锁定”更新报警状态。
  if (!armed && !alarmActive) {       // 未布防且未报警：系统待机。
    digitalWrite(LED_PIN, LOW);       // 未布防时触摸无反应，LED 保持熄灭。
    return;                           // 不检测触摸触发报警，直接返回。
  }

  if (armed && !alarmActive) {        // 已布防但还没报警：进入监视流程。
    if (millis() - armedAtMillis < ARM_IGNORE_MS) { // 布防后 800ms 内忽略触摸读数。
      digitalWrite(LED_PIN, LOW);     // 忽略窗口期间 LED 必须保持熄灭。
      return;                         // 直接返回，防止点击布防瞬间误报警。
    }

    if (isTouched()) {                // 忽略窗口结束后，只要检测到触摸就触发报警。
...
void setup() {                        // ESP32 上电或复位后执行一次。
  ...
  int marginThreshold = baseline - TOUCH_MARGIN;        // 阈值方案一：基线减固定余量。
  int ratioThreshold = baseline * TOUCH_RATIO_PERCENT / 100; // 阈值方案二：基线乘比例。
  touchThreshold = max(marginThreshold, ratioThreshold); // 取更容易触发的阈值，避免阈值过低导致摸了不报警。
  if (touchThreshold < 3) {             // 防止阈值过低。
    touchThreshold = 3;                 // 设置最小阈值。
  }
  if (touchThreshold >= baseline) {     // 防止阈值等于或超过基线，导致未触摸也报警。
    touchThreshold = baseline - 1;      // 让阈值至少比未触摸基线小一点。
  }
